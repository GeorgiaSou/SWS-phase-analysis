...
for s=1:length(sub)
    ...
    for i=1:size(data_SW,1)
        ...
        for t=1:length(tPos_allCor)
            ...
            if sigphase_degree(tPos_allCor(t),i)>=180 && sigphase_degree(tPos_allCor(t),i)<=269 || ...
               sigphase_degree(tPos_allCor(t),i)>=90 && sigphase_degree(tPos_allCor(t),i)<=179     %data_SW(tPos_allCor(t))>0  
        ...
        eval(['Sub_',num2str(s),'=struct(fields{1},sub{s},fields{2},StimCh(i),fields{3},Ntrig_phase,fields{4},mphase,fields{5},nTrig_all,fields{6},ntrig_pos,fields{7},ntrig_neg);'])
